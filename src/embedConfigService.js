...
const PowerBiReportDetails2 = require(__dirname + "/../models/embedReportConfig2.js");
const EmbedConfig = require(__dirname + "/../models/embedConfig.js");
const EmbedConfig2 = require(__dirname + "/../models/embedConfig2.js");
...
async function getEmbedInfo() {
    ...
    //Obtiene los detalles del report
    try {

        // Obtiene el embed token
...
async function getEmbedInfo2() {

    // Get the Report Embed details
    try {

        // Get report details and embed token
        const embedParams = await getEmbedParamsForSingleReport2(config.workspaceId, config.reportId2);
...
async function getEmbedParamsForSingleReport(workspaceId, reportId, additionalDatasetId) {
    ...
    // Obtiene la info del reporte llamando a la API REST de Power BI
    ...
    // Convierte los resultados a json para despues devolverlos
    ...
    // Guarda el embed token
    reportEmbedConfig.embedToken = await getEmbedTokenForSingleReportSingleWorkspace(reportId, datasetIds, workspaceId);
    return reportEmbedConfig;
}

async function getEmbedParamsForSingleReport2(workspaceId, reportId2, additionalDatasetId) {
    const reportInGroupApi = `https://api.powerbi.com/v1.0/myorg/groups/${workspaceId}/reports/${reportId2}`;
    const headers = await getRequestHeader2();
    ...
    const reportDetails2 = new PowerBiReportDetails2(resultJson.id, resultJson.name, resultJson.embedUrl);
    const reportEmbedConfig = new EmbedConfig2();

    // Create mapping for report and Embed URL
    reportEmbedConfig.reportsDetail = [reportDetails2];
    ...
    reportEmbedConfig.embedToken = await getEmbedTokenForSingleReportSingleWorkspace(reportId2, datasetIds, workspaceId);
    return reportEmbedConfig;
}
...
async function getRequestHeader2() {
